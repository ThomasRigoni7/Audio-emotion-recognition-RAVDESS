...
def spec_augment(mel_spectrogram, time_warping_para=0, frequency_masking_para=3,
    ...
    mel_spectrogram = torch.reshape(mel_spectrogram, (-1, shape[0], shape[1]))
    ...
    return warped_mel_spectrogram[0]
...
def visualization_spectrogram(mel_spectrogram, title):
    ...
    librosa.display.specshow(mel_spectrogram[0].numpy(), y_axis='mel', fmax=8000, x_axis='time')
    # plt.colorbar(format='%+2.0f dB')
    plt.title(title)
    plt.tight_layout()
    plt.savefig(pathlib.Path("plots")/ title)
