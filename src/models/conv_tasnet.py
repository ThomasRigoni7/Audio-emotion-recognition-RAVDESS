...
# Modified by: Thomas Rigoni on 2021/04
...
class ConvTasNet(nn.Module):
    ...
    def forward(self, mixture):
        ...
        # print("after encoder shape: ", mixture_w.shape)
        est_mask = self.separator(mixture_w)
        # print("after TCN shape: ", est_mask.shape)
        est_source = self.decoder(mixture_w, est_mask, multiply=True)
        # print("after decoder shape: ", est_source.shape)

        # T changed after conv1d in encoder, fix it here
        # T_origin = mixture.size(-1)
        # _conv = est_source.size(-1)
...
if __name__ == "__main__":
    ...
    M, N, L, T = 2, 3, 4, 400
    K = 2*T//L-1
    B, H, P, X, R, C, norm_type, causal = 2, 3, 3, 3, 2, 8, "gLN", False
    mixture = torch.randn((M, T))
    # test Encoder
    encoder = Encoder(L, N)
    encoder.conv1d_U.weight.data = torch.randn(encoder.conv1d_U.weight.size())
    ...
    print("TCN output shape:", est_mask.shape)
    #print('est_mask', est_mask)

    # test Decoder
    decoder = Decoder(N, L, C)
    est_source = decoder(mixture_w, est_mask, multiply=True)
    #print('est_source', est_source)

    # test Conv-TasNet
    conv_tasnet = ConvTasNet(256,20,256,512,3,8,4,8,"gLN", 0, "relu")
